...
import modelo.MenuvgaDAO;
import modelo.MenuvgaDTO;

/**
 * Servlet implementation class servletmenuvga
 */
@WebServlet("/servletmenuvga")
public class servletmenuvga extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public servletmenuvga() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("btnins") != null) {
			if (request.getParameter("tipo_menu") != "" && request.getParameter("tm_nombre") != ""&& request.getParameter("nombre_menu") != ""
				...
				mdto= new MenuvgaDTO(tipo_menu,tm_nombre ,nombre_menu, lista_ingredientes, calorias,
						precio);
				mdao = new MenuvgaDAO();
				res = mdao.insertarmenu(mdto);
				if (res.equals("r")) {
					request.setAttribute("mensaje_success", "menu registrado correctamente!");
					request.getRequestDispatcher("menu_vegano.jsp").forward(request, response);
				} else {
					request.setAttribute("mensaje_error", "menu no registrado");
					request.getRequestDispatcher("menu_vegeno.jsp").forward(request, response);
				}
		...
		if (request.getParameter("btncon") != null) {
			...
			if (request.getParameter("tipo_menu") != "") {
				...
				if (recdatos != null) {
					...
					response.sendRedirect("menu_vegano.jsp?do=" + doc +"&&nm=" + tm_nombre+ "&&rs=" + nombre_menu + "&&nc="
		...
		if (request.getParameter("btnact") != null) {
			...
			if (request.getParameter("tipo_menu") != "") {
				...
				mdto = new MenuvgaDTO(tipo_menu,tm_nombre,nombre_menu, lista_ingredientes,calorias,precio);
